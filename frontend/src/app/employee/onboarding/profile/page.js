...
function InfoField({ icon, label, value }) {
    return (
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: '10px', marginBottom: '20px' }}>
            <span style={{ fontSize: '15px', color: 'var(--text-secondary)', marginTop: '2px' }}>{icon}</span>
            <div>
    ...
    return (
        <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '260px 1fr', gap: '20px', alignItems: 'start' }}>
                {/* Left card */}
                <div style={{ background: 'var(--panel-bg)', borderRadius: '12px', border: '1px solid var(--border-color)', padding: '28px 20px', boxShadow: '0 1px 4px rgba(0,0,0,0.04)', textAlign: 'center' }}>
                    <div style={{
                    ...
                    <span style={{ display: 'inline-block', background: '#f1f5f9', color: 'var(--text-secondary)', padding: '4px 12px', borderRadius: '20px', fontSize: '12px', fontWeight: '600' }}>
                        {onboarding.employeeCode}
                    </span>
                </div>

                {/* Right info panel */}
                <div style={{ background: 'var(--panel-bg)', borderRadius: '12px', border: '1px solid var(--border-color)', padding: '24px', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
