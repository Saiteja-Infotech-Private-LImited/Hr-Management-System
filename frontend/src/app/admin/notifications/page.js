...
export default function AdminNotificationsPage() {
  ...
  // Backend notification data already contains referenceType/referenceId.
  const handleNotificationClick = async (notification) => {
    try {
      ...
      if (!referenceId) {
        toast.error('This notification is not linked to an approval request.');
        return;
      }
      ...
      if (
        ...
        router.push(
          `/admin/onboarding/document-requests?highlightId=${encodeURIComponent(referenceId)}`
        );
        return;
      }

      toast('This notification does not have an approval destination.');
    } catch {
            ...
            <div className="divide-y divide-slate-100 dark:divide-slate-800/80">
              {notifications.map((n) => (
                <div
                  ...
                  className={`flex items-start gap-3.5 sm:gap-4 p-4 sm:p-5 transition cursor-pointer ${
                    n.isRead
